...
// this code is not faster !
   ...
   * @param v The other vector in the dot product */
	...
	{
#if defined BT_USE_SIMD_btVector3 && defined (BT_USE_SSE_IN_API) && defined (BT_USE_SSE)
   ...
   * @param v The other vector */
	...
	{
        float radToLocal = v.normalized().dot(normalized());

        if(radToLocal > 1.f) radToLocal = 1.f;
        if(radToLocal < -1.f) radToLocal = -1.f;

        return btAcos(radToLocal);
	}
...
{
#if (defined BT_USE_SSE && defined BT_USE_SIMD_btVector3 && defined BT_USE_SSE_IN_API) || defined (BT_USE_NEON)
...
///btSwapbtVector3Endian swaps vector endianness, useful for network and cross-platform serialization
...
///btSwapbtVector3Endian swaps vector endianness, useful for network and cross-platform serialization
SIMD_FORCE_INLINE void	btSwapbtVector3Endian(const btVector3& sourceVec, btVector3& destVec)
{
	for (int i=0;i<4;i++)
	{
...
///btUnSwapbtVector3Endian swaps vector endianness, useful for network and cross-platform serialization
SIMD_FORCE_INLINE void	btUnSwapbtVector3Endian(btVector3& vector)
{
